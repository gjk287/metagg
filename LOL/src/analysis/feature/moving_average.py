...
def ma_feature(df, col_to_use, game_past=1):
            ...
            for season in temp_year['season'].unique():
                ...
                for team in temp_season['corresponding_team'].unique():
                    ...
                    for i in range(game_past):
                        temp_unique_team[[f'{x}__{i+1}' for x in col_to_use]] = temp_unique_team[col_to_use].shift(i)
...
def cum_opp_win_rate_feature(df):
    ...
    for league in df['league_name'].unique():
        ...
        for team in temp_league['corresponding_team'].unique():
            ...
            for opp_team in temp_season['opp_team'].unique():
                temp_team_opp = temp_team[(temp_team['team_1']==opp_team) | (temp_team['team_2']==opp_team)]
